...
function getRelatorioConsultores(PDO $pdo, array $c, string $i, string $f): array
{
    ...
    $sql = "SELECT u.no_usuario, DATE_FORMAT(f.data_emissao, '%Y-%m') AS periodo, SUM(f.valor - (f.valor * (f.total_imp_inc / 100))) AS receita_liquida, MAX(s.brut_salario) AS custo_fixo, SUM((f.valor - (f.valor * f.total_imp_inc / 100)) * (f.comissao_cn / 100)) AS comissao FROM cao_usuario AS u JOIN cao_os AS o ON u.co_usuario = o.co_usuario JOIN cao_fatura AS f ON o.co_os = f.co_os LEFT JOIN cao_salario AS s ON u.co_usuario = s.co_usuario WHERE f.data_emissao BETWEEN ? AND ? AND u.co_usuario IN ($p) GROUP BY u.co_usuario, u.no_usuario, periodo ORDER BY u.no_usuario, periodo;";
...
// --- Funções para a Aba CLIENTES (COM A CORREÇÃO NA EXECUÇÃO) ---
